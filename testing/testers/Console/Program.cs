...
class Program
{
    ...
    static async Task<bool> ProcessCommand(string input)
    {
        ...
        switch (cmd)
        {
            ...
            case "get_code":
                if (parts.Length < 2)
                {
                    PrintError("Usage: script_code <component_id>");
                    break;
                }
                await GetScriptCode(parts[1]);
                break;
            ...
            case "set_code":
                if (parts.Length < 3)
                {
                    PrintError("Usage: script_set <component_id> <code> [compile]");
                    break;
                }
                await SetScriptCode(parts[1], parts[2], parts.Length > 3 && parts[3].ToLowerInvariant() == "true");
                break;
            ...
            case "compile":
                if (parts.Length < 2)
                {
                    PrintError("Usage: script_compile <component_id>");
                    break;
                }
                await CompileScript(parts[1]);
                break;
            ...
            case "test_compile":
                if (parts.Length < 2)
                {
                    PrintError("Usage: script_test <component_id>");
                    break;
                }
                await TestScriptCompilation(parts[1]);
                break;
    ...
    static async Task GetScriptComponents()
    {
        EnsureConnected();
        var response = await _client!.GetScriptComponentsAsync();
        PrintResponse(response);
    }

    static async Task GetScriptCode(string componentId)
    {
        EnsureConnected();
        var response = await _client!.GetScriptCodeAsync(componentId);
        PrintResponse(response);
    }

    static async Task SetScriptCode(string componentId, string code, bool compile)
    {
        EnsureConnected();
        var response = await _client!.SetScriptCodeAsync(componentId, code, compile);
        PrintResponse(response);
    }

    static async Task CompileScript(string componentId)
    {
        EnsureConnected();
        var response = await _client!.CompileScriptAsync(componentId);
        PrintResponse(response);
    }

    static async Task TestScriptCompilation(string componentId)
    {
        EnsureConnected();
        var response = await _client!.TestScriptCompilationAsync(componentId);
        PrintResponse(response);
    }
    ...
    static void PrintHelp()
    {
        ...
        System.Console.WriteLine();
        System.Console.WriteLine("Script Components:");
        System.Console.WriteLine("  scripts                   - List all C# script components");
        System.Console.WriteLine("  script_code <id>          - Get script source code");
        System.Console.WriteLine("  script_set <id> <code> [c]- Set script code (c=compile)");
        System.Console.WriteLine("  script_compile <id>       - Compile script component");
        System.Console.WriteLine("  script_test <id>          - Test compilation with logs");
        System.Console.WriteLine();
