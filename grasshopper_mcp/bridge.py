...
# Set Grasshopper MCP connection settings.
GRASSHOPPER_HOST = "localhost"
GRASSHOPPER_PORT = 8080  # Default port; change as needed.
...
def send_to_grasshopper(command_type: str, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Send a command to Grasshopper MCP."""
    if params is None:
        params = {}
...
def add_component(component_type: str, x: float, y: float):
    ...
    """
    # Handle common component name ambiguities.
    component_mapping = {
        # Possible inputs for Number Slider.
        "number slider": "Number Slider",
        "numeric slider": "Number Slider",
        "num slider": "Number Slider",
        "slider": "Number Slider",  # If only "slider" is mentioned in a numeric context, default to Number Slider.
    ...
    # Check and normalize the component type.
...
def connect_components(source_id: str, target_id: str, source_param: str = None, target_param: str = None, source_param_index: int = None, target_param_index: int = None):
    ...
    """
    # Get target component info and check whether connections already exist.
    target_info = send_to_grasshopper("get_component_info", {"componentId": target_id})
    
    # Check component type; for multi-input components (Addition, Subtraction, etc.), choose inputs intelligently.
    if target_info and "result" in target_info and "type" in target_info["result"]:
        ...
        # For multi-input components, auto-select the correct input port.
        if component_type in ["Addition", "Subtraction", "Multiplication", "Division", "Math"]:
            # If no target parameter is specified and the first input is already connected,
            # automatically connect to the second input.
            if target_param is None and target_param_index is None:
                # Check whether the first input is occupied.
...
def get_component_info(component_id: str):
    ...
    # Enhance the result with more parameter information.
    if result and "result" in result:
        ...
        if "type" in component_data:
            ...
            # Query the component library for detailed parameters of this type.
            ...
            if "components" in component_library:
                for lib_component in component_library["components"]:
                    if lib_component.get("name") == component_type or lib_component.get("fullName") == component_type:
                        # Merge library parameter info into the result.
            ...
            if component_type == "Number Slider":
                # Try to get current slider settings from component data.
...
def get_all_components():
    ...
    # Enhance results with more parameter info per component.
    if result and "result" in result:
        ...
        for component in components:
            if "id" in component and "type" in component:
                ...
                # Add detailed component parameter info.
                if "components" in component_library:
                    for lib_component in component_library["components"]:
                        if lib_component.get("name") == component_type or lib_component.get("fullName") == component_type:
                            # Merge library parameter info into component data.
                            if "settings" in lib_component:
                ...
                # Special handling for certain component types.
                if component_type == "Number Slider":
                    # Try to get current slider settings.
...
def get_grasshopper_status():
    ...
    try:
        ...
        # Add a summary of current parameter values for each component.
        component_summaries = []
        for component in components:
            summary = {
                "id": component.get("id", ""),
...
def get_component_library():
    ...
    # This resource provides a more comprehensive component library, including details for common components.
    return {
        "categories": [
            {
                "name": "Params",
